...
Provides granular I/O statistics broken down by backend type, object type, and context. Replaces guesswork from `pg_stat_bgwriter`.

```sql
-- I/O by backend type (find who is doing the most I/O)
SELECT backend_type, object, context, reads, writes, write_time
FROM pg_stat_io WHERE reads + writes > 0 ORDER BY reads + writes DESC;
```

High `writes` for `client backend` = backends bypassing the background writer (shared_buffers pressure). High `reads` for `client backend` = cache misses (increase `shared_buffers` or check query patterns).

## Wait Event Analysis

Wait events reveal *where* PostgreSQL processes are spending time. Essential for diagnosing performance without guessing.

```sql
-- Current wait events (what are backends waiting on right now?)
SELECT pid, wait_event_type, wait_event, state, query
FROM pg_stat_activity
WHERE wait_event IS NOT NULL AND state = 'active';

-- Sampling approach: snapshot wait events repeatedly to build a profile
...
| Wait Event Type | Common Events | Indicates |
|----------------|--------------|-----------|
| `LWLock` | `BufferContent`, `WALWrite` | Internal lock contention |
| `Lock` | `relation`, `transactionid` | Row/table lock contention |
| `IO` | `DataFileRead`, `WALSync` | Disk I/O bottleneck |
| `Client` | `ClientRead` | Waiting for client (network or app slowness) |
...
Stats accumulate since last reset or restart; check `stats_reset` timestamp. `pg_stat_statements_reset()` clears query stats; `pg_stat_reset()` clears database stats. Reset after major maintenance, config changes, or perf testing — not routinely. Prefer snapshotting stats to external monitoring over resetting. **Always confirm with a human before resetting statistics** — resetting destroys historical performance baselines and can make it harder to identify unused indexes or regressions.
