...
Each additional index adds write-path overhead because every INSERT/UPDATE/DELETE must maintain more index entries. Benchmarks show that going from a handful of indexes to 30+ on a single table can cause a 50–60% throughput drop on write-heavy workloads.
