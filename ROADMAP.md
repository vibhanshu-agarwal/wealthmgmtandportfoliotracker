...
- **End-to-End Distributed Tracing — spec complete, implementation pending:** OpenTelemetry instrumentation is already wired across all four services — W3C propagation, OTLP trace/metrics exporters (gated off by default), Kafka producer/consumer observation, and verified HTTP `traceparent` continuity across the reactive gateway boundary (`HttpTraceContextPropagationIT`). A full spec closing both previously-tracked gaps is at `.kiro/specs/observability-app-insights/`: (1) **Kafka producer→consumer trace-ID continuity** — root-caused as a test fixture defect (both existing tests hand-build an unobserved `KafkaTemplate` instead of using the auto-configured bean, not a framework limitation) and fixed as part of the spec, completing Task 11.2 of `.kiro/specs/springboot-41-springai-2-migration/`; and (2) **a telemetry sink for the active (Azure) cloud** — routed through the ACA managed OpenTelemetry agent (chosen over the GA Application Insights Java agent to keep the existing Spring/Micrometer instrumentation load-bearing) into a dedicated Application Insights workspace, provisioned via a newly-added AzAPI Terraform provider. Sized to a ₹1100/month total ceiling for `wealth-azure-prod-rg` with allowance-independent ingestion caps and a new Cost Management budget alert, plus a `common-observability` module that redacts span content before export. Not yet implemented — `tasks.md` has not been executed.
