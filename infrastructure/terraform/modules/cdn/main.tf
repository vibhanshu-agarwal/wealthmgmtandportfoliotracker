...
resource "aws_cloudfront_distribution" "main" {
  ...
  origin {
    ...
    custom_origin_config {
      ...
      # Reduce CloudFront's origin-read timeout from the default 30 s to 60 s.
      # The api-gateway sets response-timeout = 55 s (application-prod.yml), so
  ...
  ordered_cache_behavior {
    ...
    forwarded_values {
      query_string = true
      headers      = ["Authorization", "Content-Type", "Accept", "Origin", "X-Internal-Api-Key"]
