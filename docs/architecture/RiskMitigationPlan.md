...
| Authorization depth | Gateway JWT and internal-ingress/key controls protect scoped paths | The separate path-ID advisor does not compare target ID with subject. Prioritize a focused security review; no exploit/live probe performed here. |
...
| Data freshness and coverage | Daily Azure refresh Job; unavailable/stale/partial contracts; chart dedup and FX conversion | Provider loss, incorrect symbol mapping and consumer lag remain possible. A successful Job/health response is not complete asset coverage. |
