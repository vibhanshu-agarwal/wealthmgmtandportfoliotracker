...
| Shared market-price mutation | Public price POST requires a valid login and standard rate limiting; only `ro=true` is blocked | **OPEN [operator-authorization defect](../todos/backlog/public-market-price-write-authorization/README.md).** Self-signup users can change shared Mongo prices and submit events. Azure URL wiring exists; live exploitation was not tested. Prioritize removal/key-gating under separate code/deploy approval. |
...
| User-header regression assurance | Source strips spoofed identity and injects JWT subject; a unit test checks injection | Named spoofing integration tests assert non-401 only. [Direct forwarded-header regression proof](../todos/backlog/gateway-user-header-spoofing-regression-proof/README.md) is OPEN; no current header-spoofing bypass is asserted. |
