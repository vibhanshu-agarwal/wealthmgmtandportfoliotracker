...
### Requirement 11: Replace Ollama with AWS Bedrock (Claude 3 Haiku) in insight-service

**User Story:** As a platform engineer, I want insight-service to call AWS Bedrock (Claude 3 Haiku) instead of Ollama for AI inference, so that the application is demo-ready on AWS Lambda without requiring a local LLM sidecar.

**Background:** insight-service currently has three profile-scoped AI adapters: `MockAiInsightService` (default), `OllamaAiInsightService` (profile: `ollama`), and `MockBedrockAiInsightService` (profile: `bedrock` — a placeholder stub). Ollama is a local inference server that cannot run on Lambda. The `bedrock` profile slot already exists and the Terraform IAM policy already grants `bedrock:InvokeModel` on `anthropic.claude-3-haiku-20240307-v1:0` in `us-east-1`. The Ollama dependency (`spring-ai-starter-model-ollama`) is removed because it attempts to autoconfigure an Ollama connection on startup regardless of active profile, which causes unnecessary startup overhead on Lambda. Claude 3 Haiku is chosen over Sonnet for cost efficiency (~10x cheaper) with comparable quality for portfolio insight generation.
...
2. THE Insight_Service SHALL implement `BedrockAiInsightService` (replacing `MockBedrockAiInsightService`) annotated `@Profile("bedrock")`, using Spring AI `ChatClient` backed by the Bedrock Converse model `anthropic.claude-3-haiku-20240307-v1:0`.
...
5. THE Insight_Service `application-aws.yml` SHALL configure the Bedrock model ID: `spring.ai.bedrock.converse.chat.options.model: anthropic.claude-3-haiku-20240307-v1:0` and region: `spring.ai.bedrock.aws.region: us-east-1`.
6. THE `OllamaAiInsightService` and `OllamaInsightAdvisor` classes SHALL be retained (not deleted) for local development use under the `ollama` profile, with a comment noting they are local-only and not deployed to Lambda.
7. WHEN the `bedrock` profile is not active (e.g. local dev, CI), THE Insight_Service SHALL fall back to `MockAiInsightService` and `MockInsightAdvisor` as before, with no Bedrock API calls made.
...
5. THE `OllamaAiInsightService` and `MockAiInsightService` SHALL NOT be annotated with `@Cacheable` — caching applies only to the Bedrock adapters where API cost is a concern.
