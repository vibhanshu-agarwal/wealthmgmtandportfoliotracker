...
Reviewed source candidate: `d5174ee080aa01742aaa837246b2228c2827b91a` (not merged or deployed). Subsequent publication-packet commits change only this handoff; the publication branch includes those documentation updates.
...
RED/GREEN/mutation evidence is preserved in the lane reviews and coordinator ledger: gateway transport/orchestration RED cases were converted to GREEN; strict digest/job equality mutation was killed and restored; workflow artifact/mode mutations were killed; verifier diagnostic, deadline, correlation, and current-attempt mutations were killed across exactly three fix rounds. Historical fresh execution at `db90fe74` passed 316 gateway unit, 193 gateway integration, 4 real-chain, 37 insight, 37 market-data, and 208 portfolio tests, with root `integrationTest` exit 0. At the earlier reviewed verifier candidate `caf0f648`, full Gradle verification commands exited 0 with all tasks UP-TO-DATE. Coordinator checks passed: Python suites 24+14+10+73+24+33 (178 total), identity guard PASS, 38-path propagation guard PASS, actionlint v1.7.12 with checksum/invocation PASS, and `git diff --check` PASS. PR #233's first required `static-guard` run then exposed two Azure digest uploads that lacked stable step IDs and manifest classifications; `113d9146` added the two IDs and matching `playwright: false` entries. The local failure was reproduced before the fix, then the 24 sanitizer-contract tests, live sanitizer guard, 14 Azure allowlist tests, and the 178-test combined Python suite passed; focused independent review returned SPEC PASS / QUALITY APPROVED. The next run passed `static-guard` and actionlint, then `azure-image-smoke-test` exposed an unconditional test-only `:portfolio-service` dependency in Azure's intentionally trimmed build graph. `d5174ee0` made only that Wave 8 integration dependency conditional on project presence and added a non-Bash-gated regression contract; the new test was RED then GREEN, focused gateway packaging succeeded, the full-repository dry-run still included `portfolio-service`, and independent review returned SPEC PASS / QUALITY APPROVED.
