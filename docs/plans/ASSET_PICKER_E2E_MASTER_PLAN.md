...
| 8 — login-orchestrated reset | 🟡 Task 8.1 (`updatedAt` read contract) and Task 8.2a remain source-only/not deployed. Owner-approved threshold/timeouts/placement and Astra acceptance are recorded. Tasks 8.3–8.7a, including authentication orchestration/cardinality, are source-complete and independently accepted; Task 8.8b offline workflow/helper source is complete at `b59cf26` with 48 tests. Pinned actionlint v1.7.12 passed locally with SHA-256 `8aca8db96f1b94770f1b0d72b6dddcb1ebb8123cb3712530b08cc387b349a3d8` and the required shellcheck invocation. Tasks 8.8 and 8.9 remain open: no workflow run, deployment, serving read-back, manifest comparison, or live proof is claimed. [Decision record](../superpowers/plans/2026-09-06-b2-wave8-decision-record.md). | Requires Task 3/4 orchestration and real-chain evidence, Task 8.8/8.8b deployment evidence, 5.1b (merged source-only via PR #208; not deployed), and Task 8.9; production flags remain off |
