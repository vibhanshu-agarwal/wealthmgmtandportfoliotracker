...
**TASK 8.9 PREFLIGHT PROVENANCE CORRECTION — IMPLEMENTED LOCALLY, UNMERGED:** The verifier's
...
| 8 — login-orchestrated reset | 🟡 Tasks 8.1–8.8 and 8.8b are complete for Azure. Owner-authorized scoped production run `34433715705` deployed the combined gateway artifact from `main@ea34c017514532977ce08d07be3344c1c2cb065a` as `api-gateway--0000079` at exact digest `sha256:aee44edc12b03175379caf65546e04f5ca1e2cea0d8690c00190b01254f20aa1`; the current-attempt manifest and non-interference proof passed. The approved 30-minute idle threshold and 45s/10s/60s timeouts are in the deployed code. Task 8.9's per-service provenance correction is implemented locally but unmerged, with offline tests green; it binds the gateway and portfolio deployments to their distinct run/attempt/source/digest/revision records and compares both exact live revisions before writes can arm. The first read-only rehearsal matched those live identities and stopped safely at the scaled-to-zero gateway before exec, with zero HTTP requests and zero writes. [Decision record](../superpowers/plans/2026-09-06-b2-wave8-decision-record.md), [deployment evidence](../evidence/b2-task-8-8/deployment-completion-20260910.json), [provenance packet](../evidence/b2-task-8-9/deployment-provenance-20260910.json), and [rehearsal record](../evidence/b2-task-8-9/rehearsal-20260910.json). | Task 8.9 live login/reset/log-correlation proof remains open and separately gated. The source correction is not yet published; a bounded gateway wake requires a separate owner decision before rehearsal can continue past the exec probe. Task 8.8a remains AWS-only and does not apply to this Azure deployment. Production flags remain off |
