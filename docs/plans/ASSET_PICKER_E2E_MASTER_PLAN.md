...
| 9 — live integration | 🟡 Task 9.3 drafted-price integration locally verified (real gateway `/api/market/prices` for draft tickers; Playwright + Vitest); Task 9.6 demo-auth fixture already on `main`; remaining 9.x open | Task 9.3 is source/local evidence only (not deployed). Task 9.1 catalog CORS may still be in flight separately; 9.2/9.7/9.8 remain gated on B1 Wave 7. Production E2E stays open |
