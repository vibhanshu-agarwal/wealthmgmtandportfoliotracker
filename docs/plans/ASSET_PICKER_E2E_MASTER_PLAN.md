...
Track C: B2, four phases (see Prerequisites table — not one start condition)
  ...
  Production exposure: needs all six of: Wave 7's activation
    ...
    deployed to production AND its own live serving proof green
    (Task 8.9, not deployment alone), with updated_at actually
    landed, not merely owned (pass 24 addition, corrected pass 28 —
    narrowing Live Integration's own dependency, pass 23, silently
    dropped this item's only remaining carrier; deployment alone is
    invisible-by-construction for a fail-open trigger, so "deployed"
    needed strengthening to "deployed and proven" too) — see the
