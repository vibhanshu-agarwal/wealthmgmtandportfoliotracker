...
The wrapper is a preflight-only cold-start guard: it can issue at most five fixed direct health
probes, each limited to 30 seconds and separated by the production five-second interval. Its curl
...
more than five Production health probes followed by the wrapper's read-only preflight. This does
not authorize a sixth probe, execute mode, credentials, a portfolio write or cleanup, flag change,
...
the wrapper fails closed with exit `3` and that wake is consumed; the five-probe cap leaves no
spare probe. Three accepted non-blocking implementation follow-ups are tracked in the
