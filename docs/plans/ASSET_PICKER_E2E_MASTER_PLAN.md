...
were already settled in a prior brainstorm but never formalized, then corrected across twenty-two
review passes — twenty-one by Codex adversarial review, plus one internal parallel-agent audit
...
- **`.kiro/specs/asset-picker-composition/requirements.md` and `design.md`** now exist — Revision
  2 (twenty-two review passes as of 2026-08-22 — twenty-one Codex, one internal Claude audit), synthesized from decisions already settled in
...
- **A visual design exists**: five screens (Portfolio page with the entry point, Browse/draft,
  ...
  reachable by every reviewer). **Not actually checked in yet** — pass 5's audit found this whole
  spec directory, `.kiro/specs/asset-picker-composition/`, is untracked (`git ls-files` returns
  nothing for it, `git log --all` shows zero commits touching it on any branch), unlike B1's spec
  directory, which is tracked. Until it's committed — the standing plan, once a review pass comes
  back clean — it is exactly as single-machine-dependent as the "private" hosted canvas it was
  being contrasted against, not a more durable alternative to it. This master plan document itself
...
Track C: B2, four phases (see Prerequisites table — not one start condition)
  ...
  Live integration: needs B1 Wave 7 specifically (the public
    CompositionController) AND the B2 backend build above, not just
    Wave 4
  Production exposure: needs all five of: Wave 7's activation
    ...
    reset placement, presence TTL); and the login self-call timeout
    values confirmed — see the Prerequisites table above
