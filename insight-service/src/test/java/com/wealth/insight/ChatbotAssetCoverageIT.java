 ...
 *
 * <ol>
 *   <li><b>Redis data layer (req 2.1, 2.2, 3.1, 3.6):</b> {@link MarketDataService} and
 *       {@link InsightEventListener} correctly populate Redis with the latest price, a capped
 *       10-item history, and ZSET membership — for both plain and suffixed tickers. Includes
 *       24-hour stale pruning and market summary coverage.</li>
 *   <li><b>Real Kafka → listener → Redis pipeline (req 2.1, 2.2, 3.6):</b> publishing a
 *       {@link PriceUpdatedEvent} to the embedded Kafka {@code market-prices} topic and
 *       consuming it via the live {@link InsightEventListener} container populates Redis
 *       correctly. This validates the actual Kafka consumer wiring inside insight-service,
 *       not just direct method calls.</li>
 *   <li><b>Redis-backed chat endpoint (req 2.3–2.6, 3.1):</b> with Redis seeded for
 *       {@code ROSE-USD}, {@code USDCHF=X}, and {@code RELIANCE.NS}, the {@code POST /api/chat}
 *       endpoint resolves each suffixed symbol to its exact tracked form and returns its market
 *       summary. Plain-symbol chat is also verified in the same context.</li>
 * </ol>
 *
 ...
 * insight-service) requires both services in the same JVM, which is not the project's
 * integration test model. That path is covered by the combination of
 * {@code MarketDataSeedServicePropagationHardeningTest} (seed publishes correct events) and
 * the Kafka → Redis tests here (insight-service consumes and stores them correctly).
 *
...
class ChatbotAssetCoverageIT {
     ...
     * insight-service is a consumer-only service, so its auto-configured KafkaTemplate uses
     * StringSerializer. We build a separate producer here with JsonSerializer so we can
     * publish PriceUpdatedEvent records in the Kafka → listener → Redis tests.
     */
    ...
    void setUp() {
        ...
        var producerFactory = new DefaultKafkaProducerFactory<String, PriceUpdatedEvent>(
                producerProps,
    ...
    /**
     * Consumer pipeline works identically for suffixed symbols. The exact suffixed key must
     * be used — no stripping or normalization.
     */
    @Test
    ...
    void consumerPipeline_staleTickerPruning_removesOldEntries() {
        ...
        assertThat(redisTemplate.opsForZSet().score(MarketDataService.TRACKED_TICKERS_KEY, "STALE-TICKER"))
    ...
    /**
     * Publishing a {@link PriceUpdatedEvent} to the embedded Kafka {@code market-prices} topic
     * and consuming it via the live {@link InsightEventListener} populates Redis correctly for
     * a plain symbol.
     *
     * <p>This validates the actual Kafka consumer wiring inside insight-service — not just
     * direct method calls. Awaitility polls until Redis is populated or the timeout expires.
     */
    @Test
    void kafkaToRedis_plainSymbol_listenerConsumesAndPopulatesRedis() throws Exception {
        jsonKafkaTemplate.send("market-prices", "NVDA", new PriceUpdatedEvent("NVDA", new BigDecimal("900.00")));
    ...
    /**
     * Same Kafka → listener → Redis pipeline for a suffixed symbol. Validates that the
     * listener preserves the exact suffixed key in Redis without stripping or normalizing.
     */
    @Test
    void kafkaToRedis_suffixedSymbol_listenerPreservesExactKey() throws Exception {
        jsonKafkaTemplate.send("market-prices", "BTC-USD",
                new PriceUpdatedEvent("BTC-USD", new BigDecimal("64250.00")));
    ...
    /**
     * With Redis seeded for {@code ROSE-USD}, {@code POST /api/chat} resolves the suffixed
     * symbol to its exact tracked form and returns its market summary.
     *
     * <p>This is the end-to-end path: HTTP request → suffix-aware resolver →
     * Redis-backed {@link MarketDataService#getTickerSummary} → chat response.
     */
    @Test
    ...
    /**
     * Plain-symbol chat still works in the same integration context after the suffix-aware
     * resolver changes. Verifies the full path: HTTP → resolver → Redis → response.
     */
    @Test
    ...
    /**
     * An untracked suffixed symbol returns the clarification response — not no-data.
     * Validates the preservation fix (suffix fallback removed) in the same Redis-backed context.
     */
    @Test
    void chatEndpoint_untrackedSuffixedSymbol_returnsClarification() throws Exception {
        // ROSE-USD is not seeded in Redis for this test.
        mockMvc.perform(post("/api/chat")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {"message": "How is ROSE-USD doing?"}
