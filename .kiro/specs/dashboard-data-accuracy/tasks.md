...
- **Normalize `observedAt` precision** (ms or DB-supported µs) before using `(ticker, observedAt)` as an idempotency/identity key — applies to `market_price_history` uniqueness, Redis observation identity, and replay-dedup tests. **Deferral note (Task 1):** `PriceUpdatedEvent` stores `observedAt` as a plain `Instant` (no enforcement). Precision normalization is intentionally deferred to the first use site: Task 4.1 (`market_price_history` append/dedup key) and Task 8.1 (Redis observation identity). Normalizing in the record itself would violate the "pure data carrier, no behaviour" contract. Each use site must truncate to the agreed precision (milliseconds) before keying on `(ticker, observedAt)`, and their respective tests must include sub-millisecond `Instant` values to verify no precision drift.
