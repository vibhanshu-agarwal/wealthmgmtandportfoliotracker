...
- [ ] 1. Write bug condition exploration test against a committed, deterministic fixture, using the real test seam
...
- [ ] 3. Prove the sentinel-vs-authentication ordering (Property 2, P-A.2) — the property the first draft of this plan left untested
...
- [ ] 4. Implement the font-resource allowlist
  - [ ] 4.1 Create `.github/actions/sanitize-playwright-artifacts/known-frontend-trace-resources.json`
  ...
  - [ ] 4.2 Add `loadFrontendTraceResourceAllowlist(options = {})` to `sanitize.js`, parallel to `loadAllowlistMap()` (line 551) — path-injectable and exported, unlike it
    ...
    - **Validation runs here, at sanitize-time, inside this function — not as a separate CI guard, not optional.** On any of the following thirteen cases, call `reportError("::error::<CATEGORY>: <detail>")` (no secret values in the message) and return an **empty Map**:
  ...
  - [ ] 4.3 Thread the new allowlist through the verified call chain — three hops, not one
  ...
  - [ ] 4.4 Implement the authenticated bypass in `inspectZipEntry` (line 343), preserving exact existing ordering
  ...
  - [ ] 4.5 Verify task 1's bug condition test now passes
    - Re-run the SAME test from task 1 — do not write a new one
    - **EXPECTED OUTCOME**: passes — the fixture now sanitizes cleanly
    - _Requirements: A-2.1_

  - [ ] 4.6 Verify task 3's sentinel-ordering test now passes correctly
  ...
  - [ ] 4.7 Verify task 2's preservation tests still pass
    - Re-run the SAME tests from task 2, unchanged
    - _Requirements: A-3.1, A-3.2, A-3.3_

  - [ ] 4.8 Prove a mutated, unknown, or too-deep binary still fails closed (Property 3, P-A.3)
  ...
  - [ ] 4.9 Prove manifest fail-closed validation for all thirteen cases from task 4.2, using the injectable seam
    ...
    - Assert: the return value is an empty `Map`; `capturedReportError` was called exactly once with a message containing the expected `::error::<CATEGORY>` tag and no secret values; `sanitize.js` does not crash
    - _Requirements: A-2.4_

- [ ] 5. Add sanitizer-level regression coverage for the font-authentication path — not a change to the `sanitizer-canary` CI job
  - **Do not modify `ci-verification.yml` or `test/canary/fixtures/local-server.ts`.** The existing `sanitizer-canary` job (`ci-verification.yml` ~line 86) asserts exactly one report zip and that both `fallback-only` sanitize passes over `playwright-report`/`test-results` **succeed** — it is a sentinel-redaction test. Neither a font-authentication positive case nor a fail-closed negative case fits inside that contract without producing a second zip (breaking the one-zip assertion) or a nonzero sanitize exit (breaking the must-succeed assertion). Retrofitting it would put a CI workflow file into Track A's diff for a property that job was never designed to test.
  - **Positive regression test**: re-run task 1's committed fixture (real WOFF2 bytes) through `structuredScan` **with** the real production manifest (`known-frontend-trace-resources.json`, loaded via `loadFrontendTraceResourceAllowlist()`) and confirm it sanitizes cleanly — this is the fixed-code counterpart to task 1's RED test, proving the actual production manifest (not a synthetic one) authenticates the real fixture.
...
- [ ] 7. Confirm the bug condition (C-B) against source, not just its current symptom
...
- [ ] 8. Quarantine the test with an accurate RCA comment
...
- [ ] 8.1 File the follow-up backlog item for B-NG.1
...
- [ ] 10. Confirm both tracks are independently mergeable
