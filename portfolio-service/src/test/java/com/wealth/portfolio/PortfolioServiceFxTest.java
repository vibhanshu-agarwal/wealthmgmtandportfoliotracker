...
/**
 * Unit tests for {@link PortfolioService#getSummary} with a mocked {@link FxRateProvider}.
 * No Spring context, no database, no network — pure arithmetic verification.
 */
...
class PortfolioServiceFxTest {
    ...
    void totalValueConvertsEurHoldingToUsd() {
        ...
        when(jdbcTemplate.query(anyString(), any(RowMapper.class), any(Object[].class)))
                .thenReturn(List.of(
                        new HoldingValuationRow("EURASSET",
                                new BigDecimal("10"),
                                new BigDecimal("100"),
                                "EUR")
                ));
    ...
    void unavailableRatePropagatesAsFxRateUnavailableException() {
        ...
        when(jdbcTemplate.query(anyString(), any(RowMapper.class), any(Object[].class)))
                .thenReturn(List.of(
                        new HoldingValuationRow("EXOTIC",
                                new BigDecimal("1"),
                                new BigDecimal("50"),
                                "XYZ")
                ));
