...
- Name: dashboard-data.spec.ts >> Dashboard Data Integration Diagnostics >> 1. Login flow produces an authenticated session
- Location: tests\e2e\dashboard-data.spec.ts:122:7
...
- main [ref=e2]:
  - generic [ref=e3]:
    - generic [ref=e4]:
      ...
      - paragraph [ref=e6]: Use user-001 / password for local dev.
    - generic [ref=e7]:
      - generic [ref=e8]:
        - text: Username
...
> 44  |   await page.waitForURL((url) => !url.pathname.includes("/login"), { timeout: 10_000 });
  ...
  140 | 
  141 |     await loginViaUI(page);
  142 |     await page.goto(`${BASE_URL}/portfolio`);
  143 | 
  144 |     // Wait for the element to appear (skeleton → real value)
```