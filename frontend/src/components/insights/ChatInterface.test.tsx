...
describe("ChatInterface — sentiment source", () => {
  ...
  it("labels the reply with the declared source", async () => {
    mockPostChatMessage.mockResolvedValue({
    ...
    await ask();
    await waitFor(() => {
      expect(screen.getByTestId("chat-sentiment-source")).toHaveTextContent("Generated by Azure OpenAI");
    });
  });
  ...
  it("labels rule-based text as not coming from an AI model", async () => {
    ...
    await ask();
    await waitFor(() => {
      expect(screen.getByTestId("chat-sentiment-source")).toHaveTextContent("Rule-based text (no AI model)");
    });
  });
