...
// ── Rehearsal defect #5: sentiment provenance comes only from the backend's field ──

describe("ChatInterface — sentiment source", () => {
  async function ask() {
    renderWithQueryClient();
    await act(async () => {
      fireEvent.change(screen.getByTestId("chat-input"), { target: { value: "AAPL outlook?" } });
      fireEvent.click(screen.getByTestId("chat-send"));
    });
  }

  // The reply's price and change are templated from stored data; only the appended sentiment
  // comes from the declared source, so the label must not credit the model with the whole reply.
  it("attributes only the sentiment to the declared source", async () => {
    mockPostChatMessage.mockResolvedValue({
      response: "Here's what I found for Apple (AAPL). Sentiment: Neutral.",
      sentimentSource: "AZURE_OPENAI",
    });
    await ask();
    await waitFor(() => {
      expect(screen.getByTestId("chat-sentiment-source").textContent).toBe(
        "Sentiment generated by Azure OpenAI; price and change from stored market data",
      );
    });
  });

  it("labels rule-based text as not coming from an AI model", async () => {
    mockPostChatMessage.mockResolvedValue({ response: "AAPL is showing Neutral sentiment.", sentimentSource: "RULE_BASED" });
    await ask();
    await waitFor(() => {
      expect(screen.getByTestId("chat-sentiment-source").textContent).toBe(
  ...
  it.each([[undefined], [null], ["SOMETHING_NEW"]])(
    "makes no provenance claim when the source is %p",
    async (sentimentSource) => {
      mockPostChatMessage.mockResolvedValue({ response: "Here's what I found.", sentimentSource });
      await ask();
      await waitFor(() => expect(screen.getByText("Here's what I found.")).toBeInTheDocument());
      expect(screen.queryByTestId("chat-sentiment-source")).not.toBeInTheDocument();
    },
  );
});
